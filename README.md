...
Excel Mapper is a single-user Next.js app for SAP consultants who need to map source Excel workbooks into a target Excel template with deterministic, coded mapping logic. Syntax GenAI Studio's **Test Script IQ** agent remains available as an optional fallback for ambiguous cases.
...
The API key is only required when you choose **External AI Agent** mode. Paste your Syntax GenAI Studio API key into the password field in the app. It is stored only in your browser's `localStorage` and sent to the local `/api/invoke-agent` proxy for AI-agent requests. Use **Forget** to remove it from `localStorage`.
...
6. **External AI Agent** mode still uses `/api/invoke-agent`, converts workbooks to base64 data URLs, builds the agent prompt, and invokes Test Script IQ.
7. If a workbook is found, the browser opens an Easy View with sheet switching, editable cells, and an edited-file download. Otherwise, the full run response remains visible for diagnosis.
8. Use the Revise tab to describe follow-up changes. The app re-runs the currently selected mapping engine.
...
- Uploaded files are not persisted. Deterministic mode sends files only to the local Next.js route. External AI Agent mode sends files to `/api/invoke-agent`, which proxies to Syntax GenAI Studio.
...
- Ambiguous mapping instructions should either be rewritten with explicit rules or run through External AI Agent mode.
