...
function buildPlanningPrompt({
  ...
  return [
    ...
    "",
    "For SAP Cloud ALM Test Script mode:",
    "- targetSheetName MUST be \"Test Cases\"",
    "- targetStartRow MUST be 2 (this is Excel row 2, where data starts after the header in row 1)",
    "- transformations.clearTargetRowsBeforeMapping MUST be true (to remove sample rows before writing)",
    "- The mapper will ONLY write data rows starting at row 2. Do NOT include header rows or timestamp rows in your mappings.",
    "- Row 1 of the template already has the correct headers and will be preserved automatically.",
    "- Use constantValue mappings for SAP defaults such as Test Case Status = 'In Preparation' or Test Case Priority = 'Medium' when the source does not provide those fields.",
    "",
